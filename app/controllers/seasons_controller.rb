class SeasonsController < ApplicationController
  ...
  def show
    ...
    @teams = @season.divisions.collect { |div| div.teams.sort_by{|team| team.team_total}.reverse }.flatten
  ...
  def new
    ...
    @week=Week.new()
    @season.scoring_methods = ScoringMethod.all(:conditions => 'season_id is null').collect { |sm| sm.dup }
  ...
  def create
    ...
    @season = Season.new(params[:season].merge({:course_id => params[:week][:course_id]}))
    @season.handicapping_method_attributes = params[:handicapping_method]
  ...
  def update
    ...
    respond_to do |format|
      if @season.update_attributes(params[:season].merge({:course_id => params[:week][:course_id]}))
